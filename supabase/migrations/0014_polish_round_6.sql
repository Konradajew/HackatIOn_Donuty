-- =============================================================================
-- 0014 — Polish round 6: opponent nickname, answer explanations, question
--        eligibility filter, seeded-question vote backfill
-- =============================================================================


-- 1. answer_question — add q_explanation to answer_log payload
...
BEGIN
    ...
    -- Fetch question text and explanation for log
    SELECT title, category::text, COALESCE(explanation, '')
      INTO v_q_title, v_q_cat, v_q_explanation
    ...
    -- Append answer to log (with explanation)
...
-- 2. match_snapshot — add opponent nickname via profiles lookup
...
AS $$
    ...
    LEFT JOIN public.profiles  opp1 ON opp1.id = m.player1_id
    LEFT JOIN public.profiles  opp2 ON opp2.id = m.player2_id
    WHERE m.m_id = p_match_id;
$$;

GRANT EXECUTE ON FUNCTION public.match_snapshot(int, uuid) TO authenticated;


-- 3. Backfill seeded questions (author_id IS NULL) to have at least 50 yes_votes
--    so they immediately pass the eligibility filter in play_card.
--    User-submitted questions keep their existing vote counts (default 0).
UPDATE public.questions
   SET yes_votes = GREATEST(yes_votes, 50)
 WHERE author_id IS NULL;


-- 4. play_card — filter question pool to net votes >= 50, with safe fallback
CREATE OR REPLACE FUNCTION public.play_card(p_match_id int, p_slot_idx int)
RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
...
BEGIN
    ...
    -- Pick eligible question (net votes >= 50) with fallback to any question
    SELECT q_id, title, correct_answer, wrong_answers, category
      INTO v_q
      FROM public.questions
     WHERE category::text = v_cat
       AND (yes_votes - no_votes) >= 50
     ORDER BY random()
     LIMIT 1;
