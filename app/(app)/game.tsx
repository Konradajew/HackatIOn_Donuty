...
export default function GameScreen() {
  ...
  const [lastPlay, setLastPlay] = useState<{
    type: CardType; cat: string; val: number; correct: boolean;
  } | null>({ type: 'DMG', cat: 'MATH', val: 6, correct: true });
  ...
  const handleAnswer = useCallback((answerIndex: number | null) => {
    ...
    setLastPlay({ ...card, correct: isCorrect });
    if (isCorrect) {
      setOpponentHp(prev => {
        const next = Math.max(0, prev - card.val);
      ...
      <SafeAreaView style={s.safe}>
          ...
          {lastPlay ? (
            <>
              ...
              <View style={s.lastPlayRow}>
                <BattleCard type={lastPlay.type} cat={lastPlay.cat} val={lastPlay.val} w={70} sel />
                <View style={s.lastPlayMeta}>
                  <Text
                    style={[
                      s.damageText,
                      { color: lastPlay.correct ? arc.primaryContainer : arc.secondaryContainer },
                    ]}
                  >
                    {lastPlay.correct ? `−${lastPlay.val} HP` : '+6 DMG TO YOU'}
                  </Text>
                  <Text style={[s.answerResult, { color: arc.secondaryContainer }]}>
                    {lastPlay.correct ? 'ANSWER ✓ CORRECT' : 'ANSWER ✗ WRONG'}
                  </Text>
                  {lastPlay.correct && (
                    <Text style={s.comboText}>+2 COMBO</Text>
                  )}
                </View>
              </View>
            </>
        ...
        <View style={[s.cardDetail, { borderColor: accent + '88' }]}>
          <View style={s.cardDetailMeta}>
            ...
            <View style={s.spacer} />
            <Text style={[s.dmgLabel, { color: accent }]}>{card.val} DMG</Text>
          </View>
          <Pressable
