        ...
        >
                    ...
                    <View key={label} style={[s.answerCard, { borderColor: isCorrect ? C.tertiaryDim : C.surfaceContainerHigh }]}>
                      <View style={s.answerHeaderRow}>
                        <View style={[s.answerBadge, { backgroundColor: isCorrect ? C.tertiaryDim : C.surfaceContainerHigh }]}>
                          <Text style={[s.answerBadgeText, { color: isCorrect ? C.background : C.outline }]}>{label}</Text>
                        </View>
          ...
          <View style={s.statsRow}>
            <View style={s.statBox}>
              <Text style={[s.statIcon, { color: C.tertiaryDim }]}>▲</Text>
              <Text style={[s.statValue, { color: C.tertiaryDim }]}>{q.up}</Text>
              <Text style={s.statLabel}>YES</Text>
            </View>
            <View style={s.statBox}>
              <Text style={[s.statIcon, { color: C.errorContainer }]}>▼</Text>
              <Text style={[s.statValue, { color: C.errorContainer }]}>{q.down}</Text>
              <Text style={s.statLabel}>NO</Text>
            </View>
            <View style={s.statBox}>
              <Text style={[s.statIcon, { color: C.secondaryBright }]}>◆</Text>
              <Text style={[s.statValue, { color: C.secondaryBright }]}>{avg > 0 ? `${avg}/5` : '?/5'}</Text>
            ...
            <View style={s.diffDotsRow}>
              {[1,2,3,4,5].map(d => (
                ...
                >
                  <View style={[
                    s.diffDot,
                    { backgroundColor: d <= pendingDiff ? C.primaryBright : C.surfaceContainerHigh },
          ...
          <View style={s.sectionCard}>
            ...
            <View style={s.voteRow}>
              <Pressable
                style={[s.voteBtn, {
                  backgroundColor: pendingVerdict === 'up' ? C.tertiaryDim : C.surfaceContainerHigh,
                ...
                disabled={alreadyVoted}
              >
                <Text style={[s.voteBtnText, { color: pendingVerdict === 'up' ? C.background : C.tertiaryDim }]}>✓ YES</Text>
              </Pressable>
              <Pressable
                style={[s.voteBtn, {
                  backgroundColor: pendingVerdict === 'down' ? C.errorContainer : C.surfaceContainerHigh,
                ...
                disabled={alreadyVoted}
              >
                <Text style={[s.voteBtnText, { color: pendingVerdict === 'down' ? C.surfaceContainerHigh : C.errorContainer }]}>✗ NO</Text>
              </Pressable>
            </View>
          </View>
          ...
          <Pressable
            style={[s.submitVoteBtn, {
              backgroundColor: canSubmit ? C.secondaryBright : C.surfaceContainerHigh,
            ...
            disabled={!canSubmit}
          >
            <Text style={[s.submitVoteText, { color: canSubmit ? C.background : C.outline }]}>
